...
setwd(DirSRCE)
csvDEFO <- read.csv(file="raw-data-lcc-cadt-road-dev-buffer.csv", header=TRUE, sep=",")
...
tranDEFO <- melt(dfDEFO, id.vars=c("Agent","CADT_No","Buffer"))
# Rename column names
colnames(tranDEFO) <- c("Agent","CADT","BufferDist","LandCover","DefoAreaHa")
...
write.csv(tranDEFO, paste(DirTRAN, "transform-lcc-cadt-road-dev-buffer.csv", sep=""), row.names=FALSE)
